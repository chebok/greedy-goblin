...
class FlowOperatorsTest {
    ...
    fun simple(): Unit = runBlocking {
        flowOf(1, 2, 3, 4).onEach { println(it) } // операции ...
            .onEach { delay(1000) }
            .map { it + 1 }
//            .filter { it % 2 == 0 }
    ...
    fun buffering(): Unit = runBlocking {
        ...
        flowOf(1, 2, 3, 4, 5)
            ...
            .buffer(1, BufferOverflow.DROP_LATEST) // Здесь включаем буфер размером 3 элемента
...
//            .buffer(3, BufferOverflow.SUSPEND)
    ...
    fun infiniteBuilder(): Unit = runBlocking {
        val list = flow {
            ...
            .onEach { println("$it") }
            .take(100) // Попробуйте поменять аргумент и понаблюдайте за размером результирующего списка
            .toList()
